...
public class XtendJavaValidator extends XbaseWithAnnotationsJavaValidator {
	...
	public void checkClassModelInterfaceAdherance(XtendClass clazz) {
		...
		List<String> errorMessages = mvHelper.getErrorMessages();
		List<EObject> errorObjects = mvHelper.getErrorObjects();
		for (int x = 0; x < errorMessages.size(); x++) {
			if (mvHelper.strict) {
				error(errorMessages.get(x), errorObjects.get(x), null);
			} else {
				warning(errorMessages.get(x),  errorObjects.get(x), null);
			}
		}
	}
	/* /NEW */
